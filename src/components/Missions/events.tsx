...
const TheChildEvent: MissionEventComponent = ({
  ...
  const glowUniforms = useMemo(
    () => ({
      uTime: { value: 0 },
      uIntensity: { value: 0 },
      uBaseColor: { value: new THREE.Color(0xf7b0ff) },
    }),
    []
  );
  ...
  const triggerJumpscare = useCallback(() => {
    ...
    blackoutTimeoutRef.current = window.setTimeout(() => {
      ...
      onMissionFailed?.({ message: "You failed." });
  ...
  useFrame((_, delta) => {
    ...
    if (taxi) {
      ...
      lookTarget.set(taxiWorldPosition.x, ghostPosition.y, taxiWorldPosition.z);
